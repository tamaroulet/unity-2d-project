...
- **人間に残すのは「決めること」と「作ること」だけ。**
  運ぶこと・確認すること・繰り返すことを人間に残さない。
- 動き出す条件を役割ごとに固定する。
  - **人間**: 1 日 1 回。朝報を読み、決め、絵を描く。それ以外の呼び出しをしない
  - **Claude**: 結果が置かれたとき、または人間に問われたとき。指示書とレビューだけを書く
  - **Gemini**: 指示書が置かれたとき。実装と検証の実行だけを行う
  - **機械**（検査・ビルド・テスト）: 変更が起きたとき
- Claude はサイクルの途中で設計文書を書かない。設計はサイクルの外でまとめて行う。
- 検証の振り分け: **人間を待たせる検証は常駐している Unity で行う。
  無人で回す検証は起動し直す方式で CI に置く。** 両者は同時に使えない。
...
- 指示書には**そのタスク固有のことだけ**を書く。本ファイルと `docs/laws/` の内容を転記しない。
  役割・保護ファイル一覧・停止条件・報告様式は毎回読まれるので繰り返さない。
- 検証節の既定は「機械判定 全 PASS ＋ 不変条件 全緑」の 1 行とする。
  それで足りない項目（人間の目視など）だけを足す。
- レビューは異常があるときだけ書く。無ければ書かない。
...
- **専門語を使わない。** その分野を知らない人間が読んで意味が通る語彙で書く。
  内部の名称（クラス名・アセット名・機能名）を説明なしに出さない。
