...
- Architect が読んでよいファイルは次の 4 つだけ。
  - `docs/laws/`（常時。合計 150 行以内に保つ）
  - 発行中または対象の指示書（`docs/cycles/*/03-instruction.md`）
  - `02-context.md`（指示書を発行するとき）
  - `04-result.md`（レビューするとき）

  これ以外を読む必要が生じたら、**読まずに `02-context.md` の不足として Gemini へ差し戻す。**
  「念のため周辺も読む」はしない。情報収集は Gemini の仕事である。
- したがって `02-context.md` には、Architect が判断に使う実測値がすべて載っていなければならない。
  該当ファイルの引用は、パスと行番号を添えた**該当箇所のみ**。ファイル全文は貼らない。

## 知識の蓄積

- ドキュメントは内容ではなく**寿命**で分ける。永続（`docs/laws/` `docs/decisions/`）、
  プロジェクト限り（`docs/spec/` `docs/STATUS.md`）、使い捨て（`docs/cycles/`）。
  増えてよいのは `docs/laws/` だけで、それにも 150 行の上限がある。溢れたら足さずに統合する。
- `05-review.md` は末尾に必ず「`laws/` へ足すもの: あり（内容） / なし」を書く。
- **同じ型のバグに 2 枚目の指示書を書いたら、それは知識化の失敗である。**
- 次のゲームへ持ち出すのは `docs/laws/` `docs/decisions/` 本ファイル `scripts/` と Brain 層のみ。
  `docs/cycles/` と `docs/spec/` は持ち出さない。

## 人間への報告

- 節は次の 4 つのみ。全体 25 行以内を目安とする。
  1. **ゴール階層** — 大 / 中 / 小 の 3 行。各行は上位にどう効くかを添える
     - 大ゴールは**人間の言葉をそのまま引く。** AI が言い換えない（言い換えは逸脱の隠れ場所になる）
     - 小（現在の指示書）が中に、中が大に繋がらないなら、それは逸脱である
  2. **独断で決めたこと** — 人間の指示に無いのに AI が決めたものを列挙する。無ければ「なし」
  3. **状況** — 何が動き、何が止まっているか
  4. **提案** — 次の選択肢と、推奨 1 つ
- 具体（ファイル名・行番号・コミットハッシュ・実測値）は、
  **人間がそれを使って操作・判断する場合にのみ**書く。それ以外は書かない。
- 不具合の原因の詳細は、AI 側が直すなら書かない。
  人間の操作が要るか、仕様の判断が要るときにだけ書く。
- 表と箇条書きは選択肢の提示に使う。説明には使わない。
- **思考は長く、出力は短く。** 検討の過程を出力に流さない。
