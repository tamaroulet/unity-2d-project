...
`docs/instructions/` と `docs/handoff/` を `docs/cycles/` にまとめる。

```
docs/cycles/2026-09-05-06/
  01-request.md          （あれば）
  02-context.md          （あれば）
  03-instruction.md      ← docs/instructions/NN_*.md をここへ
  04-result.md
  05-review.md
```
...
`docs/instructions/` / `docs/handoff/` へのパス参照を新しいパスへ直す。

```bash
grep -rn "docs/instructions/\|docs/handoff/" docs .agents --include=*.md
```
...
| 5 | 壊れたリンクの確認 | `grep -rn "docs/instructions/\|docs/handoff/" docs .agents --include=*.md` の出力が **0 件** |
