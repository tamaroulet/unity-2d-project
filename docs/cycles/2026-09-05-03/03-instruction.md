...
docs/cycles/19_...md                6,300 tok（Architect の出力）
docs/cycles/2026-09-04-04/05-review.md   7,900 tok（Architect の出力）
docs/cycles/2026-09-04-04/03-instruction.md  ─ 19 と内容が重複
```

**332 行のコードを直すために、Architect が 14,000 トークン以上書いている。**
しかも同じ判断を `03-instruction.md` と `docs/cycles/*/03-instruction.md` に 2 回書いている。
...
`scripts/handoff.ps1` の `ask` は現在、`03-instruction.md` と `docs/cycles/*/03-instruction.md` を
両方書かせている。**`docs/cycles/*/03-instruction.md` の 1 枚だけにする。**

- `ask` のプロンプトから `03-instruction.md` の生成指示を外す
- 判断（採用案・却下理由）は指示書の冒頭セクションに統合する
- `docs/cycles/<日付>-<連番>/` に置くのは `01-request.md` / `02-context.md` / `04-result.md` /
  `05-review.md` の 4 枚
...
```
docs/cycles/*/03-instruction.md（本書以外）  README.md  docs/spec/**  docs/decisions/**
...
`docs/cycles/2026-09-05-03/04-result.md` に規約 §5 の様式で書く。
