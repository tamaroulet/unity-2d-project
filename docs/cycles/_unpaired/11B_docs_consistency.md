...
```
指示書: docs/cycles/_unpaired/11B_docs_consistency.md
...
| 項目 | 理由 |
|---|---|
| `docs/cycles/` から `git push origin main` を除去する | Part A の C-2 修正で `.md` は `ABUSE_RULES` の検査対象外になった。かつ実測 0 件で対象が存在しない |
