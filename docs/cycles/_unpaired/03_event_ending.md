...
フォルダおよび `.asmdef` の作成は、人間が行うか、または Antigravity が Unity-MCP ツール経由（エディタ操作）で自動作成する。シェルコマンド（`mkdir` 等）による作成は禁止する。
...
| # | 禁止 | 代替 |
|---|---|---|
| 1 | シェルコマンドによるフォルダ作成 | Unity-MCP 経由で作成する |
| 2 | シェルコマンドによる asmdef 操作 | Unity-MCP 経由またはエディタ整合を保って行う |
